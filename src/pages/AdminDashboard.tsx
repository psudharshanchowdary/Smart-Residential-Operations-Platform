...
const ManageRequests = ({ requests, toast }: { requests: MaintenanceRequestData[]; toast: any }) => {
  ...
  const mutation = useMutation({
    mutationFn: ({ id, status, adminNote }: { id: string; status?: string; adminNote?: string }) =>
      updateRequest(id, { status, adminNote }),
    onSuccess: () => {
  ...
  const assignTechMutation = useMutation({
    mutationFn: ({ id, name, empId, phone }: { id: string; name: string; empId: string; phone: string }) =>
      assignTechnician(id, { technicianName: name, technicianId: empId, technicianPhone: phone }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["requests"] });
        ...
        <div key={r._id} className="bg-card rounded-xl p-4 shadow-sm border border-border">
          <div className="flex flex-wrap items-start justify-between gap-2">
            <div className="flex-1 min-w-0">
              <p className="text-xs text-muted-foreground">#{r._id.slice(-6).toUpperCase()} • {r.userName}</p>
              ...
              {r.image && (
                <div className="mt-3 relative inline-block group">
                  ...
                  <button
                    onClick={() => setConfirmDeleteImage(r._id)}
                    title="Delete image"
            ...
            <select
              value={r.status}
              onChange={e => mutation.mutate({ id: r._id, status: e.target.value })}
          ...
          {editingNote?.id === r._id ? (
            <div className="mt-2 flex gap-2">
              <input
                ...
                onChange={e => setEditingNote({ id: r._id, note: e.target.value })}
              ...
              />
              <button
                onClick={() => mutation.mutate({ id: r._id, adminNote: editingNote.note })}
            ...
            <button
              onClick={() => setEditingNote({ id: r._id, note: r.adminNote || "" })}
              className="mt-2 text-xs text-primary hover:underline"
            >
              {r.adminNote ? `📝 "${r.adminNote}"` : "+ Add note for resident"}
            </button>
          )}
          ...
          {r.technicianName && (
            <div className="mt-3 p-3 bg-primary/5 border border-primary/20 rounded-xl">
              ...
              <p className="text-xs text-foreground font-medium">{r.technicianName}</p>
              <p className="text-[11px] text-muted-foreground">ID: {r.technicianId}</p>
              <a href={`tel:${r.technicianPhone}`} className="text-[11px] text-primary flex items-center gap-1 mt-0.5 hover:underline w-fit">
                <Phone className="w-3 h-3" /> {r.technicianPhone}
              </a>
            </div>
          )}
          ...
          >
            ...
            {r.technicianName ? "Reassign Technician" : "Assign Technician"}
          </button>
        </div>
      ))}
    </div>
...
const ViewPayments = ({ payments }: { payments: PaymentData[] }) => (
  <div className="space-y-3">
    {payments.map(p => (
      <div key={p._id} className="bg-card rounded-xl p-4 shadow-sm border border-border flex justify-between items-center">
        <div>
          <p className="text-xs text-muted-foreground">{p.transactionId || p._id.slice(-8).toUpperCase()} • {p.userName}</p>
          <p className="font-semibold text-foreground">${p.amount.toLocaleString("en-US", { minimumFractionDigits: 2 })}</p>
          <p className="text-xs text-muted-foreground">{p.unit} • Due {p.dueDate}</p>
          {p.paymentDate && <p className="text-xs text-success">Paid: {p.paymentDate} via {p.paymentMethod}</p>}
        </div>
...
const SendPaymentRequest = ({ residents, toast }: { residents: any[]; toast: any }) => {
  ...
  const mutation = useMutation({
    mutationFn: () =>
      createPayment({ userId, amount: parseFloat(amount), dueDate, description: description || "Maintenance Fee" }),
    onSuccess: (payment) => {
